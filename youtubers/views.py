...
def search(request):
    ...
    if 'city' in request.GET:
        ...
        if city:
            tubers = tubers.filter(city__exat=city)
            
    if 'camera_type' in request.GET:
        camera_type = request.GET['city']
        if camera_type:
            tubers = tubers.filter(camera_type__exat=camera_type)
    
    if 'category' in request.GET:
        category = request.GET['city']
        if category:
            tubers = tubers.filter(category__exat=category)
